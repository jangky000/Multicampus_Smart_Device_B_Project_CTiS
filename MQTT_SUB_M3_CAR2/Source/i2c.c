...
void I2C_Addr (unsigned char adr) {
  ...
  I2C1->DR = adr;
  while (!(I2C_sr() & 0x0002));	//Addr sent  // 1번 비트
}
...
unsigned char I2C_getbyte(unsigned char address, unsigned char cmd) {
	...
	I2C_Addr(address);	 	// Address I2C Device. (Base address is Write Address)// 슬레이브 주소1
        I2C_Write(cmd);			// Transfer Command to I2C Device (Register to be Read) //슬레이브 주소2
	I2C_Start(); 			// Repeated start bit sequence
	I2C_Addr(address+1);	// Address I2C Device. (Base address + 1 is Read Address) // 1은 R/W 값
	uc = I2C_Read(0);		// Read 1 byte without Acknowledge // ack를 기다림
